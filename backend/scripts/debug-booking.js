...
async function main() {
    console.log('🚀 Starting Booking Debugging (JS)...');
    ...
    // 2. Get Prerequisites
    const doctorLink = await prisma.doctorClinic.findFirst({
        where: { clinicId: user.clinicId },
        include: { doctor: true }
    });
    const doctor = doctorLink ? doctorLink.doctor : null;

    const patient = await prisma.patient.findFirst({
        where: {
            patientClinics: { some: { clinicId: user.clinicId } }
        }
    });
